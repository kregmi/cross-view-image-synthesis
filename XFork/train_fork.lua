-- usage example: DATA_ROOT=/path/to/data/ which_direction=a2g name=expt1 th train_fork.lua 
--
...
opt = {
   DATA_ROOT = '/path/to/train/data/folder',         -- path to images (should have subfolders 'train', 'val', etc)
   ...
   output_nc_seg = 3,           -- #  of input segmentation image channels
   ...
   name = 'a2g_fork',              -- name of the experiment, should generally be passed on the command line
   ...
   which_model_netG = 'forkG', -- selects model to use for netG, mynetG_256_64, mynetG_64_64, fuseNet_dense, fuseNet_256
...
local real_Bs = torch.Tensor(opt.batchSize, output_nc_seg, opt.fineSize, opt.fineSize)   
local fake_B = torch.Tensor(opt.batchSize, output_nc, opt.fineSize, opt.fineSize)    
local fake_Bs = torch.Tensor(opt.batchSize, output_nc_seg, opt.fineSize, opt.fineSize)   
local fake_img = torch.Tensor(opt.batchSize, output_nc + output_nc_seg, opt.fineSize, opt.fineSize)
...
local fGx = function(x)
    ...
    if opt.use_L1==1 then
       ...
       a = df_dg + df_do_AE:mul(opt.lambda)
       
       errL1_seg = criterionAE:forward(fake_Bs, real_Bs)
       df_do_AE_seg = criterionAE:backward(fake_Bs, real_Bs)
       -- b = df_do_AE_seg:mul(opt.lambda)
       b = df_do_AE_seg:mul(1)    -- lambda = 1  used for loss of semantic maps
