...
opt = {
    DATA_ROOT = '/home/user/Documents/DATASET/dayton/AB_AsBs_3c_seg',           -- path to images (should have subfolders 'train', 'val', etc)
    ...
    name = 'dayton_A2B_Seq',                -- name of experiment, selects which model to run, should generally should be passed on command line
...
opt.nThreads = 0 -- test only works with 1 thread...
print(opt)
...
opt.netG_name = opt.name .. '/' .. opt.which_epoch .. '_net_G2'
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    paths.mkdir(paths.concat(image_dir,'input_seg'))
    paths.mkdir(paths.concat(image_dir,'output_seg'))
    paths.mkdir(paths.concat(image_dir,'target_seg'))
    -- print(input:size())
    -- print(output:size())
    -- print(target:size())
    for i=1, opt.batchSize do
        image.save(paths.concat(image_dir,'input_seg',filepaths_curr[i]), image.scale(input[i],input[i]:size(2),input[i]:size(3)/opt.aspect_ratio))
        image.save(paths.concat(image_dir,'output_seg',filepaths_curr[i]), image.scale(output[i],output[i]:size(2),output[i]:size(3)/opt.aspect_ratio))
        image.save(paths.concat(image_dir,'target_seg',filepaths_curr[i]), image.scale(target[i],target[i]:size(2),target[i]:size(3)/opt.aspect_ratio))
    end
...
-- make webpage
io.output(paths.concat(opt.results_dir,opt.netG_name .. '_' .. opt.phase, 'index_seg.html'))
...
for i=1, #filepaths do
    ...
    io.write('<td><img src="./images/input_seg/' .. filepaths[i] .. '"/></td>')
    io.write('<td><img src="./images/output_seg/' .. filepaths[i] .. '"/></td>')
    io.write('<td><img src="./images/target_seg/' .. filepaths[i] .. '"/></td>')
    io.write('</tr>')
end
